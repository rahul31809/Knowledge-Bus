...
const PALETTE = [
  {
    header: "border-t-[3px] border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40",
    yearBg: "bg-indigo-50/60 dark:bg-indigo-950/20",
    divider: "border-l-2 border-indigo-100 dark:border-indigo-900",
  },
  {
    header: "border-t-[3px] border-violet-500 bg-violet-50 dark:bg-violet-950/40",
    yearBg: "bg-violet-50/60 dark:bg-violet-950/20",
    divider: "border-l-2 border-violet-100 dark:border-violet-900",
  },
  {
    header: "border-t-[3px] border-sky-500 bg-sky-50 dark:bg-sky-950/40",
    yearBg: "bg-sky-50/60 dark:bg-sky-950/20",
    divider: "border-l-2 border-sky-100 dark:border-sky-900",
  },
];
...
function MetricRow({
  ...
  return (
    <tr className="border-t border-border/40 transition-colors hover:bg-muted/20">
      <td className="w-44 py-2.5 pl-4 pr-3 text-xs font-medium text-muted-foreground">
        {label}
        {hint ? <span className="ml-1 text-[10px] opacity-50">{hint}</span> : null}
      </td>
      {values.map((v, i) => {
        ...
        return (
          <td key={i} className={`px-3 py-2.5 text-center text-xs tabular-nums ${cls}`}>
            {v ?? "—"}
          </td>
        );
      })}
...
export function FinancialComparison({ companies }: { companies: FinancialCompanyResult[] }) {
  ...
  // Valuation rows span the full year column span but only the first column has a value
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        <table className="w-full text-left">
          <thead>
            ...
            <tr className="bg-card">
              <th className="w-44 px-4 py-3 text-xs font-medium text-muted-foreground">Metric</th>
              {successful.map((company, idx) => {
                ...
                return (
                  <th
                    key={company.name}
                    colSpan={yearCount}
                    className={`px-3 py-3 text-center ${palette.header} ${idx > 0 ? palette.divider : ""}`}
                  >
                    ...
                    {company.dataNote ? (
                      <p className="text-[10px] font-normal text-muted-foreground/70">{company.dataNote}</p>
                    ) : null}
                  </th>
                );
              })}
            ...
            <tr className="border-b-2 border-border">
              <th className="bg-muted/50 px-4 py-2" />
              {successful.map((company, idx) => {
                const palette = PALETTE[idx % PALETTE.length];
                return company.years.slice(0, 3).map((yr, yIdx) => (
                  <th
                    key={`${company.name}-${yr.year}`}
                    className={`px-3 py-2 text-center text-[11px] font-semibold text-muted-foreground ${palette.yearBg} ${idx > 0 && yIdx === 0 ? palette.divider : ""}`}
                  >
                    {yr.year}
                  </th>
                ));
          ...
          <tbody>
            ...
            {activeTab === "val" && (
              <>
                <MetricRow label="Market Cap"     values={latestOnly((c) => fmtAbsolute(c.ratios.marketCap, c.currency))} />
                <MetricRow label="P/E (TTM)"      values={latestOnly((c) => fmtMultiple(c.ratios.pe))} />
                <MetricRow label="Forward P/E"    values={latestOnly((c) => fmtMultiple(c.ratios.forwardPE))} />
                <MetricRow label="P/B"            values={latestOnly((c) => fmtMultiple(c.ratios.pb))} />
                <MetricRow label="EV / EBITDA"    values={latestOnly((c) => fmtMultiple(c.ratios.evEbitda))} />
                <MetricRow label="ROE"            values={latestOnly((c) => fmtPct(c.ratios.roe))} />
                <MetricRow label="ROA"            values={latestOnly((c) => fmtPct(c.ratios.roa))} />
                <MetricRow label="D/E Ratio"      values={latestOnly((c) => fmtMultiple(c.ratios.debtToEquity))} />
                <MetricRow label="Current Ratio"  values={latestOnly((c) => fmtMultiple(c.ratios.currentRatio))} />
                <MetricRow label="Dividend Yield" values={latestOnly((c) => fmtPct(c.ratios.dividendYield))} />
                <MetricRow label="Beta"           values={latestOnly((c) => fmtMultiple(c.ratios.beta, ""))} />
              </>
            )}
          </tbody>
        </table>
      ...
      <p className="text-[11px] italic text-muted-foreground">
        ...
        "—" indicates data not reliably available · Growth rates highlighted green / red.
      </p>
    </div>
  );
}
